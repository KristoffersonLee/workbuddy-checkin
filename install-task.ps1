...
Write-Host ""
Write-Host "=== WorkBuddy 每日自动签到 安装 (v$($Script:Version)) ===" -ForegroundColor Green
...
if (-not $taskOk) {
    ...
    $escapedPath = $ScriptPath.Replace("&", "&amp;").Replace("<", "&lt;").Replace(">", "&gt;").Replace('"', "&quot;")
...
Write-Host "  版本       : v$($Script:Version)"
...
if (-not $SkipStartupVbs) {
    ...
    Write-Host "  登录自启   : 已跳过（-SkipStartupVbs）"
}
Write-Host ""
