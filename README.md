...
v3 起已修复：找不到领取按钮且界面为加油站/积分内容时，判定「今日已签到」
...
- 关键文件编码为 **UTF-8**（PS1 脚本文件为 UTF-8 with BOM，确保 PowerShell 5.1 正确解析中文）；
- 日志按月度轮转，180 天前的自动清理；
...
- **重构**：卸载脚本自删除逻辑重写，使用独立 cmd 文件 + 循环重试，避免原生命令行拼接错误
...
- **修复**：全项目版本标识统一升级至 **v4.0.0**，所有脚本、启动器、文档完全一致
- **一致性**：全项目版本号、产品名、编码、路径规则全面收敛

---

## v3.1.0 变更记录

- **修复**：全项目 6 个文件统一补上 **UTF-8 BOM**，确保 PowerShell 5.1 正确解析中文
- **维护**：全局版本标识从 v3.0.0 升级至 **v3.1.0**，三端（安装/卸载/主程序）版本信息完全一致
- **修复**：`uninstall.bat` 标题与版本显示同步更新至 v3.1.0
- **文档**：`README.md` 版本号、目录结构、变更记录同步更新至 v3.1.0
- **一致性校验**：全项目文件编码（UTF-8 with BOM）与版本号一致性检查通过
