...
.DESCRIPTION
    ...
    核心特性：
      - 已签到识别可靠：找不到领取按钮时判定为「今日已签到」并记录状态，杜绝反复打开
      - 领取点击后轮询验证（最多 20 秒），不再单次误判
      - 点击坐标自动钳制到屏幕范围内；窗口最小化/异常时先恢复再测量
      - OCR 每次调用带 15 秒超时保护，避免挂死
      - 锁文件带时间戳自愈：卡死实例超过 45 分钟后自动接管
      - 状态文件检查前置：已签到日零开销跳过
      - 失败时自动输出屏幕 OCR 文本前 15 行，方便诊断
      - 只有「本脚本启动的 WorkBuddy」或「实际执行了领取」才自动退出
...
function Get-IdleSeconds {
    ...
    $idle = [uint32]([Environment]::TickCount) - [uint32]$lii.dwTime
    if ($idle -gt [int32]::MaxValue) { return 0 }
...
function Test-LockStale {
    try {
        ...
        if ($content -match '^(\d+)\|(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})$') {
            $holderPid = [int]$matches[1]
            $lockTime = [DateTime]::ParseExact($matches[2], "yyyy-MM-dd HH:mm:ss", $null)
            $holderProc = Get-Process -Id $holderPid -ErrorAction SilentlyContinue
            if (-not $holderProc) { return $true }
            if (((Get-Date) - $lockTime).TotalMinutes -gt $LockStaleMinutes) { return $true }
            return $false
        }
        return $true
    }
...
function Acquire-Lock {
    try {
        ...
        if (Test-LockStale) {
            try {
                Write-Log "检测到卡死的签到实例（锁无存活持有者或超过 $LockStaleMinutes 分钟），接管运行..." "WARN"
...
function Click-Point {
    ...
    [WbWin32]::mouse_event(0x0002, 0, 0, 0, 0)
    Start-Sleep -Milliseconds 80
    [WbWin32]::mouse_event(0x0004, 0, 0, 0, 0)
...
function Assert-WorkBuddyWindow {
    ...
    [void][WbWin32]::ShowWindow($hwnd, 3)
...
function Assert-Foreground {
    ...
    for ($i = 1; $i -le 4; $i++) {
        ...
        [WbWin32]::keybd_event(0x12, 0, 0, [UIntPtr]::Zero)
        Start-Sleep -Milliseconds 60
        [WbWin32]::keybd_event(0x12, 0, 2, [UIntPtr]::Zero)
        Start-Sleep -Milliseconds 60
...
function Main {
    ...
    try {
        ...
        Write-Log "=== WorkBuddy 自动签到开始（桌面端）[v$($Script:Version)] ==="
